...
export async function POST(request){
 ...
 try {
  ...
  for(const item of b.items){const id=String(item.product_id||"");const quantity=Number(item.quantity);if(!isUuid(id)||!Number.isSafeInteger(quantity)||quantity<1||quantity>100||unique.has(id))return invalid("Invalid cart items");unique.add(id);items.push({id,quantity});}
  ...
  const order=await prisma.$transaction(async(tx)=>{
    ...
    const savedCart = await tx.customerCartItem.findMany({where:{customerUuid:customer.uuid},select:{productUuid:true,quantity:true}});
    const submitted = new Map(items.map(i=>[i.id,i.quantity]));
    if(savedCart.length!==items.length || savedCart.some(i=>submitted.get(i.productUuid)!==i.quantity)) {
    ...
    const lines=items.map(item=>{const p=byId.get(item.id);const price=Number(p.salePrice??p.price);if(!Number.isFinite(price)||price<0)throw new Error("Invalid product price");subtotal+=Math.round(price*100)*item.quantity;return {productUuid:p.uuid,productName:p.name,quantity:item.quantity,unitPrice:price,lineTotal:Math.round(price*100)*item.quantity/100};});
    for(const item of items){const result=await tx.product.updateMany({where:{uuid:item.id,status:true,quantity:{gte:item.quantity}},data:{quantity:{decrement:item.quantity}}});if(result.count!==1)throw new Error("A product is out of stock or quantity changed. Please update your cart.");}
    ...
    const cleared = await tx.customerCartItem.deleteMany({where:{customerUuid:customer.uuid,productUuid:{in:items.map(i=>i.id)}}});
